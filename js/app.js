...
function renderHero(hero) {
  ...
  container.innerHTML = `
    <div class="hero-badge">
      <span class="dot"></span>
      ${hero.description || 'Available for work'}
    </div>
    <h1 class="hero-name">
      Hi, I'm <span class="gradient-text">${hero.name || 'Your Name'}</span>
    </h1>
    <p class="hero-tagline">${hero.tagline || 'Developer & Designer'}</p>
    <div class="hero-actions">
      <a href="#projects" class="btn btn-primary">View My Work ${getIcon('arrow')}</a>
...
function renderAbout(about) {
  ...
  const chipsHtml = (about.chips || []).map(c => `<span class="chip">${c}</span>`).join('');

  container.innerHTML = `
    <div class="about-grid">
      <div class="about-photo-wrapper">
        <img src="${about.photo}" alt="Profile" class="about-photo" onerror="this.src='https://via.placeholder.com/400?text=Photo'"/>
      </div>
      <div>
        <p class="about-intro">${about.introduction}</p>
  ...
  container.innerHTML = (projects || []).map(p => `
    <div class="project-card">
      <div class="project-image-wrapper">
        <img src="${p.image}" alt="${p.title}" class="project-image" onerror="this.src='https://via.placeholder.com/600x400?text=Project'"/>
        <div class="project-overlay">
          ${p.liveUrl ? `<a href="${p.liveUrl}" target="_blank" class="btn btn-primary">Live Demo ${getIcon('external-link')}</a>` : ''}
          ${p.repoUrl ? `<a href="${p.repoUrl}" target="_blank" class="btn btn-ghost">Source ${getIcon('code')}</a>` : ''}
        </div>
      </div>
...
function renderFooter(hero, contact) {
  ...
  if (footerSocials && hero.socials) {
    footerSocials.innerHTML = hero.socials.map(s => `
      <a href="${s.url}" target="_blank" rel="noopener" title="${s.platform}">${s.platform}</a>
    `).join('');
  }
